# Spec: agent-docs-sync

## Purpose

Defines the contract for the upstream agent-docs sync tool — a script that reads a
`skills-manifest.yaml` to map upstream Handlebars templates under `agent-docs/` into
rendered Markdown skill files under `skills/`. Covers manifest-driven scope mapping,
helper resolution (`docLink`, `localHubLink`), variable substitution, JSX escaping,
SKILL.md rendering, idempotency, diff reporting, ignore lists, the npm script entry
point, and lint mode.
